...
@Service
public class IqiyiDetailPipeline extends JsonPipeline {
...
//			producer.send(id, new Gson().toJson(newsInfoBean));
			...
			e.printStackTrace();
			LOGGER.error("IQIYI 详情抽取错误:" + e.getMessage());
		}
	}
	...
	private boolean parseContent(NewsInfoBean newsInfoBean) {
		try {
			...
			// 获取body体标签
			Elements bodyDivs = doc.select("div.new_i_cent");
			...
			e.printStackTrace();
			LOGGER.error("IQIYI parseContent解析内容错误:" + e.getMessage());
		}
		return false;
	}
