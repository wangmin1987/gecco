...
//					break;
				...
				log.info("request2:"+request.getUrl());
				start = true;
			} else { 
				log.info("request1:"+request.getUrl());
			}
...
//					}
					...
					if(response.getStatus() == 200) {
						...
						String url = request.getParameter("original_url");
						url = url == null ? request.getUrl() : url + "," + request.getUrl();
						request.addParameter("original_url", url);
						spiderScheduler.into(request.subRequest(response.getContent()));
					}
				}
			} catch(Exception ex) {
