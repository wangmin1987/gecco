...
public class Status extends WeiboResponse {
	...
	private List<String> picUrls;                             //微博配图
	...
	private void constructJson(JSONObject json) throws WeiboException {
		try {
			...
			if (!json.isNull("pic_urls")) {
				JSONArray pic_urls = json.getJSONArray("pic_urls");
				picUrls = new ArrayList<String>(pic_urls.length());
				for (int i = 0; i < pic_urls.length(); i++) {
					picUrls.add(pic_urls.getJSONObject(i).getString("thumbnail_pic"));
				}
			} else {
				picUrls = new ArrayList<String>(0);
			}
	...
	public static StatusWapper constructWapperStatus(Response res) throws WeiboException {
		...
		try {
			...
			long sinceId = jsonStatus.getLong("since_id");
			return new StatusWapper(status, previousCursor, nextCursor,totalNumber,hasvisible,sinceId);
